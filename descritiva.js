var medsep = 0
function iniciar() {
    var range = document.querySelector('p.separatriz input')
...
function qualitativa(vetor, vet2, freq, freqR, fac, facP) {
    ...
    switch (medidaSep) {
        case 'quartil':
            separatriz = `Quartil: ${vetor[position]}`
            break;
        case 'quintil':
            separatriz = `Quintil: ${vetor[position]}`
            break;
        case 'decil':
            separatriz = `Decil: ${vetor[position]}`
            break;
        case 'porcentil':
            separatriz = `Porcentil: ${vetor[position]}`
            break;
    }
...
function quantitativa(vetor, tipoVar, tipoDados, vet2, freq, freqR, fac, facP) {
    ...
    if (tipoVar[2].checked) {
        ...
        tabela += `<th> ${separatriz} </th>`
        tabela += `<th> Variância: ${(variancia)} </th>`
        tabela += `<th> Desvio Padrão: ${(dp)} </th>`
        tabela += `<th> Coeficiente de Variação: ${(cv)} </th>`
        tabela += `</tr> </thead>`
        tabela += '</table>'
